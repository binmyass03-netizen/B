...
pcall(function()
    ...
    for name, fn in pairs(Sync) do
        if typeof(fn) ~= "function" then -- continue removed end
        if isexecutorclosure(fn) then -- continue removed end

        local ok, ups = pcall(debug.getupvalues, fn)
        if not ok then -- continue removed end
...
task.spawn(function()
    ...
    if PlayerGui:FindFirstChild("bbchubIntro") then
        PlayerGui.bbchubIntro:Destroy()
    end

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "bbchubIntro"
...
function applyTheme(themeName)
    ...
    local guiNames = {
        "AutoStealUI", "bbcAdminPanel", "SettingsUI", "StealSpeedUI",
        "bbcInvisPanel", "bbcStatusHUD", "bbcMobileControls", "bbcNotif",
        "bbcThemeUI", "PriorityListGUI", "bbcJobJoiner", "bbcPriorityAlert",
        "bbcSettings", "bbcPlatformUI"
    }
    ...
    task.spawn(function()
        local savedTab  = (_G.bbcSettingsUI and _G.bbcSettingsUI.currentTab) or "act"
        local wasVis    = _G.bbcSettingsUI and _G.bbcSettingsUI.panel and _G.bbcSettingsUI.panel.Visible
        if buildbbcSettingsUI then
            buildbbcSettingsUI()
        end
        task.wait()
        if _G.bbcSettingsUI then
            if _G.bbcSettingsUI.switchTab then
                _G.bbcSettingsUI.switchTab(savedTab)
            end
            if wasVis and _G.bbcSettingsUI.panel then
                _G.bbcSettingsUI.panel.Visible = true
            end
        end
        ...
        local guisRT = {"AutoStealUI","bbcAdminPanel","SettingsUI","StealSpeedUI","bbcInvisPanel","bbcSettings","bbcStatusHUD","bbcAutoBuyUI"}
...
if Config.UltraLightMode then
    task.spawn(function()
        ...
        for _,gn in ipairs({"AutoStealUI","bbcAdminPanel","SettingsUI","StealSpeedUI",
            "bbcInvisPanel","bbcStatusHUD","bbcSettings","bbcPlatformUI",
            "bbcAutoBuyUI","bbcMiniActions"}) do
            local sg = PlayerGui:FindFirstChild(gn)
            if sg then
                for _,obj in ipairs(sg:GetDescendants()) do
                    if obj.Name == "RacetrackBorder" and obj:IsA("UIStroke") then
...
task.spawn(function()
    ...
    local function findNearestPrompt_Instant()
        ...
        for _, plot in ipairs(plots:GetChildren()) do
            if isMyPlot_Instant(plot.Name) then -- continue removed end
            local plotDist = math.huge
            pcall(function() plotDist = (plot:GetPivot().Position - hrp.Position).Magnitude end)
            if plotDist > INSTANT_STEAL_RADIUS + 40 then -- continue removed end
            local podiums = plot:FindFirstChild("AnimalPodiums")
            if not podiums then -- continue removed end
            for _, pod in ipairs(podiums:GetChildren()) do
                local base = pod:FindFirstChild("Base")
                local spawn = base and base:FindFirstChild("Spawn")
                if not spawn then -- continue removed end
                local dist = (spawn.Position - hrp.Position).Magnitude
                if dist > INSTANT_STEAL_RADIUS or dist >= bestDist then -- continue removed end
                local att = spawn:FindFirstChild("PromptAttachment")
                if not att then -- continue removed end
    ...
    local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
        ...
        local existing = PlayerGui:FindFirstChild("bbcPriorityAlert")
        if existing then existing:Destroy() end
        
        local alertGui = Instance.new("ScreenGui")
        alertGui.Name = "bbcPriorityAlert"
...
task.spawn(function()
    ...
    refreshBtn.MouseButton1Click:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if Config.HideKawaifuFromPanel and isKawaifuUser(player) then -- continue removed end
...
task.spawn(function()
    ...
    while true do
        ...
        local char = LocalPlayer.Character
        if not char then -- continue removed end
...
task.spawn(function()
    ...
    while true do
        task.wait(1)
        if not Config.AutoResetOnBalloon then -- continue removed end
...
if IS_MOBILE then
    ...
    mobileGui.Name = "bbcMobileControls"
    ...
    createMobBtn("UI", Color3.fromRGB(255, 255, 255), 5, function()
        ...
        local adUI = PlayerGui:FindFirstChild("bbcAdminPanel")
...
CreateToggleSwitch(rShowUnlockHUD, Config.ShowUnlockButtonsHUD, function(ns, set)
    ...
    local hudGui = PlayerGui:FindFirstChild("bbcStatusHUD")
    if hudGui then
...
CreateToggleSwitch(rJoinerRow, Config.ShowJobJoiner, function(ns, set)
    ...
    local gui = PlayerGui:FindFirstChild("bbcJobJoiner")
...
bReset.MouseButton1Click:Connect(function()
    ...
    if PlayerGui:FindFirstChild("bbcAdminPanel") and PlayerGui.XiAdminPanel:FindFirstChild("Frame") then
    ...
    if PlayerGui:FindFirstChild("bbcInvisPanel") and PlayerGui.XiInvisPanel:FindFirstChild("Frame") then
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        if input.KeyCode == (Enum.KeyCode[Config.MenuKey] or Enum.KeyCode.LeftControl) then
            if _G.bbcSettingsUI and _G.bbcSettingsUI.panel then
                _G.bbcSettingsUI.panel.Visible = not _G.bbcSettingsUI.panel.Visible
            else
                settingsGui.Enabled = not settingsGui.Enabled
            end
        end
        ...
        if Config.JobJoinerKey and input.KeyCode == Enum.KeyCode[Config.JobJoinerKey] then
            local joinerGui = PlayerGui:FindFirstChild("bbcJobJoiner")
            if joinerGui then
...
task.spawn(function()
    ...
    if Config.HideInvisPanel then
        local g = PlayerGui:FindFirstChild("bbcInvisPanel")
        if g then g.Enabled = false end
    end
    if Config.HideStatusHUD then
        local g = PlayerGui:FindFirstChild("bbcStatusHUD")
        if g then g.Enabled = false end
    end
    if Config.HideAutoBuyUI then
        local g = PlayerGui:FindFirstChild("bbcAutoBuyUI")
        if g then local p = g:FindFirstChild("ABPanel"); if p then p.Visible = false end end
    end
    if Config.HidePlatformUI then
        local g = PlayerGui:FindFirstChild("bbcPlatformUI")
        if g then g.Enabled = false end
    end
...
task.spawn(function()
    ...
    task.spawn(function()
        ...
        while true do
            task.wait(0.3)
            if not Config.AutoStealSpeed then lastHadSteal = nil; -- continue removed end
            local hasSteal = (LocalPlayer:GetAttribute("Stealing") == true)
            if lastHadSteal == hasSteal then -- continue removed end
...
task.spawn(function()
    ...
    brainrotESPFolder.Name = "bbcBrainrotESP"
...
task.spawn(function()
    ...
    local function buildHUD()
        local existing = PlayerGui:FindFirstChild("bbcStatusHUD")
        if existing then existing:Destroy() end

        local hudGui = Instance.new("ScreenGui")
        hudGui.Name = "bbcStatusHUD"
...
task.spawn(function()
    local function makeBlacklistESP(player)
        ...
        local old = char:FindFirstChild("bbcBlacklistESP")
        if old then old:Destroy() end

        local msg = Config.BlacklistMsg or "BLOCKED"

        local bb = Instance.new("BillboardGui")
        bb.Name                  = "bbcBlacklistESP"
    ...
    local function removeBlacklistESP(player)
        local char = player.Character
        if char then
            local bb = char:FindFirstChild("bbcBlacklistESP")
    ...
    while true do
        task.wait(0.5)
        if Config.BlacklistESP == false then -- continue removed end
        for _, p in ipairs(Players:GetPlayers()) do
            if p == LocalPlayer then -- continue removed end
            pcall(function()
                if isBlacklisted and isBlacklisted(p.Name) then
                    local char = p.Character
                    if char and not char:FindFirstChild("bbcBlacklistESP") then
                        makeBlacklistESP(p)
                    end
                else
                    removeBlacklistESP(p)
...
-- ============================================================
function buildbbcSettingsUI()
    local pg = PlayerGui
    if not pg then return end
    local oldBSG2 = pg:FindFirstChild("bbcSettings")
    if oldBSG2 then oldBSG2:Destroy() end

    local bsg = Instance.new("ScreenGui")
    bsg.Name = "bbcSettings"
    ...
    do
        local sp = Config.bbcSettingsPos
        if sp then
    ...
    local function switchTab2(id)
        activTab2 = id
        if _G.bbcSettingsUI then _G.bbcSettingsUI.currentTab = id end
    ...
    makeBtn(aS, "Painel Admin",            4, function()
        local g = PlayerGui:FindFirstChild("bbcAdminPanel")
    ...
    makeBtn(aS, "Job Joiner",              6, function()
        local g = PlayerGui:FindFirstChild("bbcJobJoiner")
        if g then g.Enabled = not g.Enabled end
    end)
    ...
    do
        ...
        ulBtn.MouseButton1Click:Connect(function()
            ...
            if ns then
                ...
                for _,gn in ipairs({"AutoStealUI","bbcAdminPanel","SettingsUI","StealSpeedUI","bbcInvisPanel","bbcStatusHUD","bbcSettings","bbcPlatformUI","bbcAutoBuyUI","bbcMiniActions"}) do
    ...
    makeToggle(uhS,"Hide Admin Panel",function() return Config.HideAdminPanel end,function(v) Config.HideAdminPanel=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcAdminPanel"); if g then g.Enabled=not v end end,11)
    ...
    makeToggle(uhS,"Hide Auto Buy UI",function() return Config.HideAutoBuyUI end,function(v) Config.HideAutoBuyUI=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcAutoBuyUI"); if g then local p=g:FindFirstChild("ABPanel"); if p then p.Visible=not v end end end,13)
    ...
    makeToggle(uhS,"Hide Status HUD",function() return Config.HideStatusHUD end,function(v) Config.HideStatusHUD=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcStatusHUD"); if g then g.Enabled=not v end end,15)
    makeToggle(uhS,"Hide Invis Panel",function() return Config.HideInvisPanel end,function(v) Config.HideInvisPanel=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcInvisPanel"); if g then g.Enabled=not v end end,16)
    makeToggle(uhS,"Hide Platform UI",function() return Config.HidePlatformUI end,function(v) Config.HidePlatformUI=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcPlatformUI"); if g then g.Enabled=not v end end,17)
    ...
    makeToggle(uhS,"Mostrar Mini UI",function() return Config.ShowMiniActions end,function(v) Config.ShowMiniActions=v; SaveConfig(); local g=PlayerGui:FindFirstChild("bbcMiniActions"); if g then local mp=g:FindFirstChild("MiniPanel"); if mp then mp.Visible=v end end end,19)
    ...
    makeToggle(blS, "Show Blacklist ESP", function() return Config.BlacklistESP ~= false end, function(v)
        ...
        if not v then
            for _, p in ipairs(Players:GetPlayers()) do
                local char = p.Character
                if char then
                    local existing = char:FindFirstChild("bbcBlacklistESP")
                    if existing then existing:Destroy() end
                end
            end
        end
    ...
    -- Expor ref global
    _G.bbcSettingsUI = {panel=panel, switchTab=switchTab2, currentTab="act"}
end
task.spawn(buildbbcSettingsUI)
...
function buildMiniActionsUI()
    local pg = PlayerGui
    if not pg then return end
    local oldG = pg:FindFirstChild("bbcMiniActions")
    if oldG then oldG:Destroy() end

    local maGui = Instance.new("ScreenGui")
    maGui.Name = "bbcMiniActions"
    ...
    local _oldAB = PlayerGui:FindFirstChild("bbcAutoBuyUI")
    if _oldAB then _oldAB:Destroy() end

    local abGui = Instance.new("ScreenGui")
    abGui.Name         = "bbcAutoBuyUI"
    ...
    task.spawn(function()
                ...
                if bb:IsA("BillboardGui") then
                    for _, lbl in ipairs(bb:GetDescendants()) do
                        if lbl:IsA("TextLabel") and lbl.Text and lbl.Text ~= "" then
                            ...
                            local tl = t:lower()
                            if RARITY_WORDS[tl] then -- continue removed end
                            if t:match("^%$[%d%.]+[KkMmBb]?/s$") then
                                if genFound=="" then genFound=t end; -- continue removed
                            end
                            if t:match("^%$[%d%.]+[KkMmBb]?$") then -- continue removed end
                            if t:match("^[%d%.]+[KkMmBb]?$") then -- continue removed end
        ...
        local function scanConveyor()
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if not (obj:IsA("ProximityPrompt") and obj.Enabled) then -- continue removed end
                local txt = obj.ActionText or ""
                if not (txt=="Purchase" or txt:lower():find("purchase") or txt:lower():find("comprar")) then -- continue removed end
                local part = obj.Parent
                if not part then -- continue removed end
                local realPart = part:IsA("Attachment") and part.Parent or part
                if not (realPart and realPart:IsA("BasePart")) then -- continue removed end
        ...
        task.spawn(function()
            while true do
                ...
                if not autoBuyActive then -- continue removed end
                if not partAlive()   then -- continue removed end
                if promptAlive() then
        ...
        task.spawn(function()
            while true do
                ...
                local char = LocalPlayer.Character
                local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                if not hrp then -- continue removed end
...
task.spawn(function()
    ...
    if Config.AutoHideMiniUI then
        local g = PlayerGui:FindFirstChild("bbcMiniActions")
...
if owner.UserId == LocalPlayer.UserId then return end
        ...
        for slot,ad in pairs(al) do
            if type(ad)=="table" then
                local nm=ad.Index
                local info=AnimalsData and AnimalsData[nm]; if not info then -- continue removed end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- en AUTO mode. On garde le pct actuel et on -- continue removed depuis là.
    if target~=stealTarget then
        ...
        if not target then
            ...
            -- Si "charging" : -- continue removed de charger normalement
        end
    end
